...
(joint_data) with r n_obs monthly observations across r n_zip_unique
unique ZIP codes and r n_nbhd unique neighborhoods. Key variables are
zip_code, date, and price (ZORI), with location metadata such as
borough, county, and neighborhood. ZIP codes in the ZIP file but not in
the Zillow rental dataset (zips_missing) are likely non-residential zip
codes, so Zillow did not publish an index. Comparing January 2020 to
January 2021, the table largest10_drop shows the 10 ZIP codes with the
largest rent declines, along with their borough and neighborhood.
